...
- 📈 Система аналитических отчетов с возможностью фильтрации по периоду и знаку зодиака
- 📉 Графическая визуализация статистики предсказаний с интерактивными диаграммами
- 📊 Анализ тональности предсказаний (позитивные, негативные, нейтральные)
- 📋 Экспорт отчетов в CSV-формат с поддержкой русского языка
...
- **Django ORM**: Система для аналитических запросов и формирования отчетов
...
5. **Система отчетов**:
   - Доступ к отчетам через кнопку "Отчеты" в админ-панели раздела "Предсказания"
   - Формирование аналитических отчетов с настраиваемыми параметрами
   - Выбор периода и знака зодиака для анализа данных
   - Экспорт отчетов в CSV-формат для дальнейшей обработки

### 📊 Доступные типы отчетов

| Тип отчета | Описание | Параметры фильтрации |
|------------|----------|----------------------|
| Количество предсказаний по знакам зодиака | Показывает распределение предсказаний по знакам зодиака в виде диаграммы и таблицы | Период, знак зодиака |
| Количество предсказаний по датам | Отображает динамику генерации предсказаний по датам | Период, знак зодиака |
| Статистика счастливых чисел | Анализирует частоту появления различных счастливых чисел в предсказаниях | Период, знак зодиака |
| Анализ тональности предсказаний | Классифицирует предсказания по эмоциональному окрасу на позитивные, негативные и нейтральные | Период, знак зодиака |
| Детальный отчет по предсказаниям | Предоставляет полную информацию о всех предсказаниях с возможностью экспорта | Период, знак зодиака |

### 📋 Работа с отчетами

1. **Формирование отчета**:
   - Выберите тип отчета
   - Укажите интересующий знак зодиака или выберите "Все знаки"
   - Задайте временной период для анализа
   - Нажмите кнопку "Сформировать отчет"

2. **Визуализация данных**:
   - Результаты представлены в виде интерактивных диаграмм
   - Табличное представление данных для более детального анализа

3. **Экспорт отчетов**:
   - Нажмите кнопку "Экспорт в CSV" для сохранения результатов
   - Файл содержит все данные в формате, совместимом с Excel и другими табличными редакторами
   - Полная поддержка русского языка и правильное форматирование данных в экспортированных файлах
...
erDiagram
    ...
    %% Логические связи с системой отчетов (виртуальные сущности)
    HoroscopePrediction ||--o{ Reports : "используется в"
    ZodiacSign ||--o{ Reports : "используется в"
    Reports {
        string type "Тип отчета"
        date start_date "Начало периода"
        date end_date "Конец периода"
        int sign_id "Фильтр по знаку"
    }
```
...
|------|-----|----------|--------|
| id | Integer | Первичный ключ | 1 |
| sign_id | Integer (FK) | Ссылка на знак зодиака | 1 |
| date | Date | Дата предсказания | "2023-12-25" |
| prediction | TextField | Общее предсказание | "Сегодня вам будет сопутствовать удача..." |
| love | TextField | Предсказание о любви | "В любовных отношениях возможны приятные события..." |
| career | TextField | Предсказание о карьере | "На работе вас ждет повышение..." |
| health | TextField | Предсказание о здоровье | "Уделите внимание своему физическому состоянию..." |
| lucky_number | Integer | Счастливое число дня | 7 |
| created_at | DateTime | Дата создания записи | "2023-12-24 20:15:32" |

#### 3. **Prediction (Шаблон предсказания)**

Содержит шаблонные тексты для генерации предсказаний при недоступности GigaChat API.

| Поле | Тип | Описание | Пример |
|------|-----|----------|--------|
| id | Integer | Первичный ключ | 1 |
| category | CharField | Категория предсказания | "general", "love", "career", "health" |
| text | TextField | Текст шаблона | "Сегодня звезды рекомендуют вам..." |

#### 4. **Reports (Система отчетов)**

Виртуальная сущность, представляющая логику генерации аналитических отчетов по данным гороскопов.

| Параметр | Тип | Описание | Пример |
|----------|-----|----------|--------|
| type | String | Тип отчета | "predictions_by_sign", "sentiment_analysis" |
| start_date | Date | Начало периода анализа | "2023-01-01" |
| end_date | Date | Конец периода анализа | "2023-12-31" |
| sign_id | Integer | Фильтр по знаку зодиака (опционально) | 1 или "all" |

#### Доступные типы отчетов:

- **predictions_by_sign**: Количество предсказаний по знакам зодиака
- **predictions_by_date**: Количество предсказаний по датам 
- **lucky_numbers**: Статистика счастливых чисел
- **sentiment_analysis**: Анализ тональности предсказаний
- **detailed_predictions**: Детальный отчет по предсказаниям
